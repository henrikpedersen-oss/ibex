...
`define CE `CR.g_cheriot_ex.u_ibex_cheriot_ex
// ibex-private has no TBRE/TRVK stage; stub it with the local module below
`define TRVK trvk_stub_i
...
module top import ibex_pkg::*; #(
  ...
  // HeapBase/TSMapSize: used by revocation properties; ibex-private has no tsmap HW,
  // so revocation properties are vacuously true but must compile.
  ...
  // [32]=tag bit (ibex-private data_tag_o), [31:0]=data (ibex-private data_wdata_o)
...
localparam logic [31:0] CSR_MVENDORID_CHERI_VALUE = 32'b0;
localparam logic [31:0] CSR_MIMPID_VALUE          = 32'b0;
localparam logic [31:0] CSR_MARCHID_CHERI_VALUE   = 32'hce1;
// perms_t.EX sits at bit 8; psgen.sv uses the old integer-index style
localparam int unsigned PERM_EX = 8;

// ibex_top lockstep/shadow outputs not in formal top's port list — declare as local sinks
...
ibex_top #(
    ...
    // ibex-private splits wdata and tag; map to combined data_wdata_o[32:0]
    .data_wdata_o(data_wdata_o[31:0]),
    .data_tag_o(data_wdata_o[32]),
    // all other ports match by name
    .*
);

// ibex-private has no tsmap (TBRE) port — declare as free local signals
// so that mem.sv/psgen.sv compile without errors; revocation properties are vacuously true.
logic [15:0] tsmap_addr_o;
logic [31:0] tsmap_rdata_i;

// ibex submodule adds trvk_revbm_* and trvk_heap_base_addr_i for TBRE revocation bitmap;
// not verified in this formal flow — declare free sinks/sources for .* binding.
...
// ibex-private has no TBRE/TRVK stage — stub with a zero-valued module instance
// so that psgen.sv properties referencing TRVK signals can elaborate.
cheri_trvk_stub trvk_stub_i();
...
// 4. CHERI mode enable
CheriConfig: assume property (cheriot_enable_i == IbexMuBiOn);

// See protocol/* for further assumptions

///////////////////////////////// Declarations /////////////////////////////////

// alt_lsu.sv uses a plain-logic cheri_pmode_i; derive it from cheriot_enable_i
logic cheri_pmode_i;
assign cheri_pmode_i = (cheriot_enable_i == IbexMuBiOn);
...
`define X_EACH_CSR_NC \
    ...
    // Omitted: nmi_mode, mstack, mstack_epc, mstack_cause
    // The current ibexspec (old CHERIoT RV32E build) does not have these ports.
    // `X(nmi_mode) `X(mstack) `X(mstack_epc) `X(mstack_cause)
    // `X(mshwmb) `X(mshwm)
