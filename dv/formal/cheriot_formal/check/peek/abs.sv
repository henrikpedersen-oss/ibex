...
// CHERIoT mode uses g_cheriot_rf, which is 16 entries (x0-x15) matching the RV32E spec.
for (genvar i = 0; i < 16; i++) begin: g_pre_regs
    t_Capability rf_data;
    // ibex-private has no TBRE: revocation always 0
    assign rf_data = regcap2sail(`RF.g_cheriot_rf.rf_shared[i], `RF.g_cheriot_rf.rf_data[i]);
    assign pre_regs[i] = `CR.rf_write_wb && `CR.rf_waddr_wb == i && i > '0 ? fwd_data : rf_data;
end
// x16-x31 do not exist in CHERIoT mode; the spec has no ports for them.
for (genvar i = 16; i < 32; i++) begin: g_pre_regs_unused
    assign pre_regs[i] = nullCap();
end
...
// mstack/mstack_cause/mstack_epc omitted: old ibexspec lacks these ports
...
always_comb begin
    ...
    else if ((cheriot_enable_i == IbexMuBiOn) & `CSR.mepc_en)
        mepc_cap_next <= NULL_CAP;
    else if (`CSR.mepc_en_cheriot)
        mepc_cap_next <= `CSR.cheriot_csr_wcap_i;
    else
        mepc_cap_next <= `CSR.mepc_cap;
end

t_Capability csr_incoming_cap;
assign csr_incoming_cap = regcap2sail(`CSR.cheriot_csr_wcap_i, `CSR.cheriot_csr_wdata_i);
...
assign post_mtvec = `CSR.mtvec_en_combi ?
    regcap2sail(`CSR.mtvec_en_cheriot ? `CSR.cheriot_csr_wcap_i : `CSR.mtvec_cap, `CSR.mtvec_d_combi):
    pre_mtvec;
assign post_mepc = `CSR.mepc_en_combi ? regcap2sail(mepc_cap_next, `CSR.mepc_d_combi) : pre_mepc;
assign post_mtdc = `CSRG.mtdc_en_cheriot ? csr_incoming_cap : pre_mtdc;
assign post_mscratchc = `CSRG.mscratchc_en_cheriot ? csr_incoming_cap : pre_mscratchc;
assign post_pcc = pcc2sail(`CSR.pcc_cap_d, pcc_address_d);
// post_mstack/post_mstack_cause/post_mstack_epc omitted: old ibexspec lacks these ports
...
for (genvar i = 1; i < 16; i++) begin: g_post_trvk_regs
    // ibex-private has no TBRE: revocation always 0
    assign post_trvk_regs[i] = regcap2sail(`RF.g_cheriot_rf.rf_shared[i], `RF.g_cheriot_rf.rf_data[i]);
end
