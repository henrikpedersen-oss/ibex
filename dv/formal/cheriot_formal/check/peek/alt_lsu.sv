...
module alt_lsu import ibex_pkg::*; import ibex_cheriot_pkg::*; #(
  ...
  input cap_clrperm_t resp_lc_clrperm_q,
  ...
  if (CHERIoTEn & ~MemCapFmt) begin : gen_memcap_rd_fmt0
    ...
    assign lsu_rcap_o  = (resp_is_cap_q && data_rvalid_i && (cap_rx_fsm_q == CRX_WAIT_RESP2) && (~data_or_pmp_err)) ?
                         cheriot_mem_to_cap(data_rdata_i, cap_lsw_q, resp_lc_clrperm_q) : NULL_CAP;
  end else if (CHERIoTEn) begin : gen_memcap_rd_fmt1
    assign lsu_rdata_o = data_rdata_ext; // MemCapFmt=1 not used in formal
    assign lsu_rcap_o  = (resp_is_cap_q && data_rvalid_i && (cap_rx_fsm_q == CRX_WAIT_RESP2) && (~data_or_pmp_err)) ?
                         cheriot_mem_to_cap(data_rdata_i, cap_lsw_q, resp_lc_clrperm_q) : NULL_CAP;
